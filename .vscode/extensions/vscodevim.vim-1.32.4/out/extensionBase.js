...
async function activate(context, handleLocal = true) {
    ...
    if (document) {
        ...
        register_1.Register.setReadonlyRegister('%', filepathComponents.at(-1));
    }
    // workspace events
    ...
    registerEventListener(context, vscode.workspace.onDidChangeTextDocument, async (event) => {
        ...
        for (const change of event.contentChanges) {
            ...
            if (change.text.length > 0) {
                globalState_1.globalState.jumpTracker.handleTextAdded(event.document, change.range.start, change.text);
            }
        }
    ...
    registerEventListener(context, vscode.window.onDidChangeActiveTextEditor, async (activeTextEditor) => {
        ...
        taskQueue_1.taskQueue.enqueueTask(async () => {
            ...
            if (mh) {
                globalState_1.globalState.jumpTracker.handleFileJump(lastClosedModeHandler ? jump_1.Jump.fromStateNow(lastClosedModeHandler.vimState) : undefined, jump_1.Jump.fromStateNow(mh.vimState));
            }
        });
    }, true, true);
    registerEventListener(context, vscode.window.onDidChangeTextEditorSelection, async (e) => {
        ...
        if (mh.vimState.currentMode === mode_1.Mode.EasyMotionMode) {
            return;
        }
